...
package com.relative.chat.bot.ia.infrastructure.ai.http;
...
public class HttpEmbeddingsClient implements EmbeddingsPort {
    ...
    @Override public float[] embedOne(String text) { return embedMany(java.util.List.of(text)).get(0); }
    @Override public List<float[]> embedMany(List<String> texts) {
        ...
        return resp.data().stream().map(list -> {
            ...
            for (int i=0;i<list.size();i++) f[i] = list.get(i).floatValue();
            return f;
        }).toList();
    }
}
